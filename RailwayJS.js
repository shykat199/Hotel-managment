 //On hovering the mouse over a certain image only that image should be clear and rest images to be blurred
function mouseover() {
	var mouseOver=document.querySelector('#img');
	mouseOver.classList.add('blur');
}
function mouseout () {
	var mouseOut=document.querySelector('#img');
	mouseOut.classList.remove('blur');
}
function blurImages(event){
	...
	for (var i = 0; i < image.length; i++) {
		if (event.target!=image[i]) {
			...
			{
				image[i].style.filter='blur(5px)'; 
			}
		}
	}
}
document.addEventListener('mouseover', blurImages);
